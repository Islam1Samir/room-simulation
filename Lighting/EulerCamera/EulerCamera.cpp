...
{
	if(check(mPosition+(dist *mDirection)))
...
{
	if(check(mPosition+(dist *mRight)))
...
bool EulerCamera::check(glm::vec3 mPosition2)
{
	if((mPosition2[0]>=-1.8&&mPosition2[0]<=1.8&&mPosition2[2]<=2&&mPosition2[2]>=-1&&mPosition2[1]>=0&&mPosition2[1]<1.9)|| (mPosition2[0] >= 1.2&&mPosition2[2] <= -0.4))
		if((mPosition2[2]<1&&mPosition2[0]>=-0.5)||mPosition2[2]==-1||mPosition2[2]>=1)
